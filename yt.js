(function(window) {
  ...
  $youtube.fn = $youtube.prototype = {
    ...
    $.ajax({
     ...
     success: function(data) {
      ...
      var title = "",
       ...
       numRaters, k = 0,
       publishedAt = "",
       dthumbnail, mthumbnail, hthumbnail, sthumbnail, categoryId = "",
       channelTitle = "",
       hr, mins, secs;
      ...
      if (data.items[0].contentDetails) {
       ...
       if (length > 1) mins = parseInt(matches[length - 2]);
       else mins = 0;
       if (length > 2) hr = parseInt(matches[length - 3]);
       else hr = 0;
      ...
      if (data.items[0].contentDetails) {
       licensedContent = data.items[0].contentDetails.licensedContent;
      } else {
   ...
   search: function(keyword, maxResults, fullResults, order, navigate, extraparam) {
    ...
    if (fullResults) {
     if (fullResults == "yes") fullResults = 1;
     else fullResults = 0;
    }
    if (!order) {
     order = "Relevance";
    ...
    if (navigate) {
     if (navigate == "next") {
      if (this.videos.nextPageToken) navigate = "&pageToken=" + this.videos.nextPageToken;
      else return this.ErrorAll(21);
     } else if (navigate == "prev") {
      if (this.videos.prevPageToken) navigate = "&pageToken=" + this.videos.prevPageToken;
      else {
       return this.ErrorAll(22);
      }
     } else navigate = "";
    }
   ...
   standard: function(type, startindex, maxResults) {
    ...
    if (type == "displayOptions") {
     return this.availableOptions(0);
    }
    if (startindex) {
     if (startindex > 0 && startindex < 1000) {} else return this.ErrorAll(0);
    } else {
     startindex = 1;
    }
    if (!maxResults) {
